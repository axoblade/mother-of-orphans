...
export async function sendPasswordResetEmail(to: string, otp: string): Promise<void> {
	...
	await send({
		...
		text: `Your password reset code is: ${otp}\n\nThis code expires in 10 minutes. Enter it at: ${loginUrl}\n\nIf you did not request this, ignore this email.`,
		html: `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px">
  ...
  <a href="${loginUrl}" style="display:inline-block;background:#ed6221;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:600">Go to Login Page</a>
  <p style="color:#888;font-size:13px;margin-top:24px">If you did not request a password reset, you can safely ignore this email.</p>
</div>`,
	});
}
